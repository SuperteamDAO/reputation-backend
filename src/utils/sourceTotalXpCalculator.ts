...
function remove_duplicates_and_add_value(
  ...
  const s = new Set(arr.map((item) => item.skill));
  const it = s.values();
  const unique_skills = Array.from(it);
  const unique_skills_with_amount = unique_skills.map((skill) => {
    let amount = 0;
    arr.forEach((item) => {
      if (item.skill === skill) {
        amount += item.amount;
...
export function overallXPDetails(xpSources: xpType[], name: string, overallXP: totalOverallXPType): xpType {
  const total_xp = overallXP?.total_xp;
  ...
  const xp: { amount: number[]; dates: Date[] } = { amount: [], dates: [] };
  xpSources.forEach((xpSource) => {
    if (!xpSource?.name || !xpSource?.total_amount || !xpSource?.skills || !xpSource?.xp) return;
    name = xpSource.name;
...
export function xpSumPerPersonCalculator(person: {
  ...
  const skills: { skill: skillKind; amount: number }[] = remove_duplicates_and_add_value(skillsArr);
...
export function skillTotalXPCalculator(source_xp: allocated_xp[], skill: string) {
  // sum all the allocated xp
  const total_xp_for_provided_skill = source_xp.reduce((accumulator: any, currentValue: any) => {
...
export function sourceTotalXPCalculator(
  ...
  xp_source_data?.forEach((xp_source) => {
    const index = sourceTotalXPArr.findIndex((item) => item.name === xp_source.name);
    if (index === -1) {
